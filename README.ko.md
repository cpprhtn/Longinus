...
가볍고 정밀합니다. 도구가 빠져 있다면 [tool preflight](references/tooling/tool-preflight.md)로
`ready|docker|missing|blocked` 상태를 기록하고, 가능할 때 Docker fallback을 사용하세요.
...
| 출시 전/바이브코딩 repo의 저비용 1차 점검 | [references/basic-vibe-triage.md](references/basic-vibe-triage.md) |
...
└── references/                     ← 실제 플레이북 트리
    ...
    ├── basic-vibe-triage.md                                  ← 기본 계정/debug/admin 표면 저비용 1차 점검

...
2. **Profile.** Identify the stack + form factor (skill → `references/audit-modes.md` first-contact
   profiling). Decide which domains are in play. Pick a mode (quick/standard/deep).
...
7. **Report.** One ranked deliverable: executive summary → prioritized fix list → per-finding detail.
   ...
   (`references/enforce-forward.md`). **Lead with the chain, then the parts.** State what was *not* tested.
